# frozen_string_literal: true

require "spec_helper"

# Minimal font-like struct for testing CpMap's fallback scan path.
# Not a double — a real data container that answers the same protocol
# CpMap's scan_cmap uses (responds to #table("cmap") → #unicode_mappings).
class CpMapCoverageFontStub
  def initialize(cmap_data)
    @cmap_data = cmap_data
  end

  def table(name)
    return nil unless name == "cmap"

    Struct.new(:unicode_mappings).new(@cmap_data)
  end
end

RSpec.describe Essenfont::CpMap, "restrict_to_covers enforcement" do
  it "uses donor.coverage instead of rescanning donor.font" do
    # leaky donor has Basic Latin in its RAW cmap, but coverage (filtered)
    # only includes CJK. CpMap reads coverage directly — it never touches
    # the raw font's cmap. Coverage scanning is DonorLoader's job.
    leaky_raw_cmap = { 0x41 => 1, 0x42 => 2, 0x4E00 => 3 }
    clean_raw_cmap = { 0x41 => 1 }

    donors = {
      "leaky" => Essenfont::Donor::Info.new(
        label: "leaky",
        font: CpMapCoverageFontStub.new(leaky_raw_cmap),
        coverage: { 0x4E00 => 3 },
        remap: nil
      ),
      "clean" => Essenfont::Donor::Info.new(
        label: "clean",
        font: CpMapCoverageFontStub.new(clean_raw_cmap),
        coverage: { 0x41 => 1 },
        remap: nil
      )
    }

    cp_map = Essenfont::CpMap.from_donors(donors)

    # If CpMap read the RAW cmap, leaky would win 0x41 (first-wins).
    # Reading coverage, clean wins 0x41.
    expect(cp_map[0x41][:label]).to eq("clean")
    expect(cp_map[0x4E00][:label]).to eq("leaky")
  end
end
